    ...
    return (
      <Paper className={classes.paper} elevation={6}>
        <Typography variant="h6" align="center">
          Por favor inicia sesión para crear, comentar y valorar publicaciones.
        </Typography>
      </Paper>
    );
  }
  ...
  return (
    <Paper className={classes.paper} elevation={6}>
      <form autoComplete="off" noValidate className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
        <Typography variant="h6">{currentId ? `Editando "${post?.title}"` : 'Crear una publicación'}</Typography>
        ...
        <div style={{ padding: '5px 0', width: '94%' }}>
          <ChipInput
            name="tags"
            variant="outlined"
            label="Etiquetas (presionar Enter)"
            fullWidth
            value={postData.tags}
        ...
        <Button variant="contained" color="secondary" size="small" onClick={clear} fullWidth>Vaciar campos</Button>
      </form>
    </Paper>
  );
};
